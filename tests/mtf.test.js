...
function makeSetup(overrides = {}) {
  const entryScore = overrides.entryScore ?? 60;
  const regimeScore = overrides.regimeScore ?? 0;
...
describe('applyMTFScoring — confluence bonus', () => {
  it('single setup per coin gets no bonus', () => {
    const setups = [makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 60, timeframe: '15m' })];
    const result = applyMTFScoring(setups);
    expect(result[0].entryScore).toBe(60);
    expect(result[0].mtfConfluence).toBeNull();
  });

  it('2 TFs same direction → weighted bonus (15m+1h = 8 pts)', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 60, timeframe: '15m' }),
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 65, timeframe: '1h' }),
    ];
    const result = applyMTFScoring(setups);
    // Best entryScore is 65 → 65+8=73
    expect(result[0].entryScore).toBe(73);
  });

  it('3 TFs same direction → sum of all pairs with cap 25 (5m+15m+1h = 6+10+8=24)', () => {
    const setups = [
      makeSetup({ coin: 'ETH', dir: 'buy', entryScore: 60, timeframe: '5m' }),
      makeSetup({ coin: 'ETH', dir: 'buy', entryScore: 65, timeframe: '15m' }),
      makeSetup({ coin: 'ETH', dir: 'buy', entryScore: 70, timeframe: '1h' }),
    ];
    const result = applyMTFScoring(setups);
    // getMTFWeight(5m,15m)=6, getMTFWeight(5m,1h)=10, getMTFWeight(15m,1h)=8 → total=24, capped at 25 → 24
    // best entryScore = 70+24=94
    expect(result[0].entryScore).toBe(94);
  });

  it('4+ TFs same direction caps bonus at 25 (5m+15m+1h+4h pairs sum >> 25)', () => {
    const setups = [
      makeSetup({ coin: 'SOL', dir: 'sell', entryScore: 60, timeframe: '5m' }),
      makeSetup({ coin: 'SOL', dir: 'sell', entryScore: 60, timeframe: '15m' }),
      makeSetup({ coin: 'SOL', dir: 'sell', entryScore: 60, timeframe: '1h' }),
      makeSetup({ coin: 'SOL', dir: 'sell', entryScore: 60, timeframe: '4h' }),
    ];
    const result = applyMTFScoring(setups);
    // pairs: 5m+15m=6, 5m+1h=10, 5m+4h=14, 15m+1h=8, 15m+4h=12, 1h+4h=10 → total=60, cap=25
    // 60+25=85
    expect(result[0].entryScore).toBe(85);
  });

  it('entryScore is uncapped after bonus (can exceed 100)', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 96, timeframe: '15m' }),
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 97, timeframe: '1h' }),
    ];
    const result = applyMTFScoring(setups);
    // 97 + 8 = 105 — no cap now
    expect(result[0].entryScore).toBe(105);
  });

  it('mtfConfluence has hardTFs, softTFs, bonus, dir, count, tfs', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 60, timeframe: '15m' }),
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 70, timeframe: '1h' }),
    ];
    const result = applyMTFScoring(setups);
  ...
  it('confluence reason is prepended to reasons array', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'sell', entryScore: 60, timeframe: '15m' }),
      makeSetup({ coin: 'BTC', dir: 'sell', entryScore: 70, timeframe: '1h' }),
    ];
    const result = applyMTFScoring(setups);
  ...
  it('buy and sell setups for same coin do not cross-influence each other', () => {
    ...
    // dedup uses |entryScore| now; |70| > |60-20|=40, so sell wins
    expect(result).toHaveLength(1);
  });

  it('soft confirms contribute to bonus but appear in softTFs not hardTFs', () => {
    const hardSetups = [
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 70, timeframe: '15m' }),
    ];
    const softSetups = [
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 60, timeframe: '1h' }),
    ];
    ...
    expect(mtf.bonus).toBe(8); // getMTFWeight(15m,1h)=8
    // entryScore: 70 + 8 = 78
    expect(result[0].entryScore).toBe(78);
  });
  ...
  it('soft confirms for wrong coin are ignored', () => {
    ...
    const softSetups = [
      makeSetup({ coin: 'ETH', dir: 'buy', entryScore: 60, timeframe: '1h' }), // different coin
    ];
    const result = applyMTFScoring(hardSetups, softSetups);
...
describe('applyMTFScoring — conflict penalty', () => {
  it('lower TF opposing higher TF with gap >= 2 gets -20 penalty (15m vs 1h, gap=2)', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'buy',  entryScore: 60, timeframe: '15m' }), // lower TF, opposing
      makeSetup({ coin: 'BTC', dir: 'sell', entryScore: 70, timeframe: '1h' }),  // higher TF
    ];
    applyMTFScoring(setups);
    // TF_ORDER: 15m=idx1, 1h=idx3, gap=2 → penalty=20
    // The buy/15m setup should have entryScore reduced by 20 → 60-20=40
    expect(setups[0].entryScore).toBe(40);
  });

  it('lower TF opposing adjacent TF (gap=1) gets -8 penalty (5m vs 15m)', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'buy',  entryScore: 60, timeframe: '5m' }),  // lower TF, gap=1
      makeSetup({ coin: 'BTC', dir: 'sell', entryScore: 70, timeframe: '15m' }), // higher TF
    ];
    applyMTFScoring(setups);
    // TF_ORDER: 5m=idx0, 15m=idx1, gap=1 → penalty=8
    expect(setups[0].entryScore).toBe(52); // 60-8=52
  });
  ...
  it('higher TF does not get conflict penalty', () => {
    ...
    expect(setups[1].entryScore).toBe(70);
  });

  it('conflict penalty is signed and can go negative (uncapped)', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'buy',  entryScore: 10, timeframe: '15m' }), // will be penalized -20
      makeSetup({ coin: 'BTC', dir: 'sell', entryScore: 80, timeframe: '4h' }),
    ];
    applyMTFScoring(setups);
    // 10 - 20 = -10 (no floor at 0 anymore)
    expect(setups[0].entryScore).toBe(-10);
  });

  it('conflict reason text includes "Conflito"', () => {
    const setups = [
      makeSetup({ coin: 'ETH', dir: 'buy',  entryScore: 60, timeframe: '15m' }),
      makeSetup({ coin: 'ETH', dir: 'sell', entryScore: 70, timeframe: '1h' }),
    ];
    applyMTFScoring(setups);
  ...
  it('same direction on all TFs gets no conflict penalty', () => {
    const setups = [
      makeSetup({ coin: 'SOL', dir: 'buy', entryScore: 60, timeframe: '15m' }),
      makeSetup({ coin: 'SOL', dir: 'buy', entryScore: 70, timeframe: '1h' }),
    ];
    applyMTFScoring(setups);
    // buy/15m should NOT be penalized — same direction as buy/1h
    // 2 TFs same direction → getMTFWeight(15m,1h)=8 bonus
    expect(setups[0].entryScore).toBe(68); // 60 + 8 bonus
  });
});
...
describe('applyMTFScoring — deduplication', () => {
  it('returns only one setup per coin', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 60, timeframe: '15m' }),
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 70, timeframe: '1h' }),
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 65, timeframe: '4h' }),
    ];
    const result = applyMTFScoring(setups);
  ...
  it('keeps the highest-|entryScore| setup after bonus/penalty are applied', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 60, timeframe: '15m' }),
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 70, timeframe: '1h' }),
    ];
  ...
  it('different coins are not deduplicated together', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 60, timeframe: '15m' }),
      makeSetup({ coin: 'ETH', dir: 'buy', entryScore: 70, timeframe: '15m' }),
    ];
    const result = applyMTFScoring(setups);
...
describe('applyMTFScoring — sorting', () => {
  it('results are sorted by m3.cap.netPct descending', () => {
    const setups = [
      makeSetup({ coin: 'BTC', dir: 'buy', entryScore: 60, timeframe: '15m', _capNetPct: '30.00' }),
      makeSetup({ coin: 'ETH', dir: 'buy', entryScore: 70, timeframe: '15m', _capNetPct: '50.00' }),
      makeSetup({ coin: 'SOL', dir: 'buy', entryScore: 65, timeframe: '15m', _capNetPct: '40.00' }),
    ];
    const result = applyMTFScoring(setups);
