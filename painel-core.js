...
const TF_INTERVAL_MS = { '5m':300000, '15m':900000, '30m':1800000, '1h':3600000, '4h':14400000, '1D':86400000 };
...
async function fetchCandles(symbol, tf, signal = null) {
  ...
  const candles = j.result.list.reverse().map(c => ({
    time: parseInt(c[0]), open: parseFloat(c[1]),
    high: parseFloat(c[2]), low: parseFloat(c[3]),
    close: parseFloat(c[4]), volume: parseFloat(c[5]),
  }));
  // Bybit returns the in-progress candle as the most recent. Pattern detectors
  // and indicators run on the last element, so a partial candle produces false
  // positives (e.g. Marubozu triggering on a 24-second-old candle with body≈range).
  const intervalMs = TF_INTERVAL_MS[tf];
  if (intervalMs && candles.length && candles[candles.length-1].time + intervalMs > Date.now()) {
    candles.pop();
  }
  return candles;
}
...
function _computeScore(price, ind, fg, fundingRate = null, openInterest = null) {
  ...
  // Snapshot direction before applying bonuses that amplify rather than determine bias.
  // Using the running score here would make Volume/ADX/OI results order-dependent.
  const scoreDir = score >= 0 ? 1 : -1;

  const vp = ((volRatio-1)*100).toFixed(0);
  if (volRatio > 1.5) { score += scoreDir * 7; reasons.push({text:`Volume +${vp}% acima da média`,type:scoreDir>0?'positive':'negative'}); }
  ...
  if (adx !== null) {
    if      (adx > 30) { score += scoreDir * 10; reasons.push({text:`ADX ${adx.toFixed(1)} — Tendência muito forte`,type:scoreDir>0?'positive':'negative'}); }
    else if (adx > 25) { score += scoreDir * 6;  reasons.push({text:`ADX ${adx.toFixed(1)} — Tendência forte`,type:scoreDir>0?'positive':'negative'}); }
    else if (adx < 20) { score -= 8; reasons.push({text:`ADX ${adx.toFixed(1)} — Mercado lateral`,type:'negative'}); }
    else               { score -= 3; reasons.push({text:`ADX ${adx.toFixed(1)} — Tendência fraca`,type:'negative'}); }
    const adxColor = adx > 25 ? 'var(--accent)' : adx < 20 ? 'var(--danger)' : 'var(--warn)';
    indicators.push({name:'ADX (14)',reading:`${adx.toFixed(1)} — ${adx>30?'Tendência muito forte':adx>25?'Tendência forte':adx<20?'Mercado lateral':'Tendência fraca'}`,color:adxColor});
  }
  ...
  if (openInterest !== null) {
    ...
    if (oiChg > 5) {
      score += scoreDir * 8;
      const tipo = scoreDir > 0 ? 'Capital novo entrando (confirma long)' : 'Capital novo no short (confirma)';
      reasons.push({ text: `OI ${oiStr} — ${tipo}`, type: scoreDir > 0 ? 'positive' : 'negative' });
    } else if (oiChg < -5) {
      score -= 6;
...
function analyzeCandles(coin, tf, candles, fg, fundingRate = null, openInterest = null, options = { score: '0', leverage: 10, rr: 'fib' }, news = []) {
  ...
  const TF_ADX_MIN = { '5m': 23, '15m': 22, '30m': 20, '1h': 18, '4h': 18, '1D': 18 };
