...
function _computeScore(price, ind, fg, fundingRate = null, openInterest = null) {
  ...
  if (adx !== null) {
    if      (adx > 30) { score += (score>=0?10:-10); reasons.push({text:`ADX ${adx.toFixed(1)} — Tendência muito forte`,type:score>=0?'positive':'negative'}); }
    else if (adx > 25) { score += (score>=0?6:-6);   reasons.push({text:`ADX ${adx.toFixed(1)} — Tendência forte`,type:score>=0?'positive':'negative'}); }
