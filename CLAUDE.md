...
- **Stop mínimo e máximo por timeframe:** `analyzeCandles` rejeita setups onde o stop final está fora da faixa permitida por TF:
  - `TF_MIN_STOP` (5m:0.8%, 15m:1.2%, 30m:1.5%, 1h:2%, 4h:3%, 1D:5%) — previne stop hunts em alavancagem alta
  - `TF_MAX_STOP` (5m:2.0%, 15m:2.0%, 30m:2.5%, 1h:3.5%, 4h:5.5%, 1D:8.0%) — rejeita setups com ATR muito alto (stops largos amplificam perdas desproporcionalmente com alavancagem, ex: 15m stop 3.5% + 10x = -36.5% capital)
...
- **OBV direction-aware:** `_computeScore` aplica OBV apenas quando confirma a direção do setup. OBV em ascensão com score < 0 (SHORT) = sinal contraditório, registrado como `type:'neutral'` sem pontuar. Idem OBV em queda com score > 0 (LONG). Backtest mostrou 38% WR quando OBV contradiz SHORT — pior que aleatório.
